...
public class ReceiptDAO extends DBContext {
    ...
    public int sumDebt(int year, int month, int lodgingHouseId) {
        String sql = "SELECT SUM(Price) money FROM Receipt WHERE StatusPayMent = 0 AND YEAR(DateTime) = ? AND MONTH(DateTime) = ?";
        if (lodgingHouseId != 0) {
    ...
    public List<Integer> getYearDebt() {
        String sql = " SELECT DISTINCT Year(MonthYear) [year] FROM Bill B full JOIN Receipt R on B.MonthYear = R.DateTime WHERE YEAR(MonthYear) iS NOT NULL";
    ...
    public static void main(String[] args) {
        ...
        System.out.println(dao.sumDebt(2024, 1, 3));
    }
